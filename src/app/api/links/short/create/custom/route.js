...
export async function POST(request) {
  ...
  try {
    const { link, keyword, usageLimit, CustomExpDate, collectionId } =
      await request.json();
    ...
    if (filterStatus === 1) {
      console.log(
        `[WARN][SHORT_ROUTE] UserId: ${user.id} tried to short a banned url: ${link}`,
      );
    ...
    if (existingUUID) {
      console.log(
        `[WARN][SHORT_ROUTE] UUID (${UUID}) already exists, creating new UUID`,
      );
      UUID = generateUUID(5);
    }

    expDate =
      CustomExpDate.length > 0
        ? new Date(
            new Date().setMonth(
              new Date().getMonth() + parseInt(CustomExpDate),
            ),
          )
        : null;
